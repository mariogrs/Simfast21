...
int main(int argc, char **argv){
  ...
  double complex *map_out1, *map_out2, *pwsp3d;
  fftw_plan pr2c;
  int flag;
  double wsum;
...
#ifdef _OMPTHREAD_
  if(argc != 10) {
    printf("SYNTAX: power3d_cross Nbytes N1 N2 N3 dl flag input_file1 input_file2 nthreads (output goes to standard output)\n\n");
    printf("\"Nbytes\" is the number of bytes of each value (e.g., 4 for float and 8 for double).\n# The input files should have the following format: N3 is the fastest index, then N2, then N1 (e.g. the first set of numbers in the input binary file should be for N3 and so on...).\n# dl is the cell size (the same in all directions). The k and P(k) will have units after dl.\n# flag selects the window function: 0 - top hat, 1 - Bartlett, 2 - Welch\n");
    exit(1);
  }
...
#else
  if(argc != 9)  {
    printf("SYNTAX: power3d_cross Nbytes N1 N2 N3 dl flag input_file1 input_file2 (output goes to standard output)\n\n");
    printf("\"Nbytes\" is the number of bytes of each value (e.g., 4 for float and 8 for double).\n# The input files should have the following format: N3 is the fastest index, then N2, then N1 (e.g. the first set of numbers in the input binary file should be for N3 and so on...).\n# dl is the cell size (the same in all directions). The k and P(k) will have units after dl.\n# flag selects the window function: 0 - top hat, 1 - Bartlett, 2 - Welch\n");
    exit(1); 
  }
#endif
  ...
  printf("# Input parameters - Nbytes:%d, N1:%ld,  N2:%ld, N3:%ld, dl:%f, flag:%d, L1:%f, L2:%f, L3:%f, dk:%f, input files:%s, %s\n", bsize, N1, N2, N3, dl, flag, L1, L2, L3, dk,argv[7],argv[8]);
  
  nk=(long int)(round(sqrt(((N1/2)*(N1/2))+((N2/2)*(N2/2))+((N3/2)*(N3/2)))))+1;
  nps=(long int *)malloc(nk*sizeof(long int));
  pwsp3d=(double complex*)malloc(nk*sizeof(double complex));
  for(i=0;i<nk;i++) {
    nps[i]=0;
    pwsp3d[i]=0.;
  }
...
#pragma omp parallel for shared(map_in_w,map_in_d,N1,N2,N3) private(i,j,p)
  ...
  if(!(map_out1 = (double complex*) malloc(sizeof(double complex) * N1*N2*(N3/2+1)))) {
    printf("Mem4 Problem...\n");
    exit(1);
  }
  if(!(pr2c=fftw_plan_dft_r2c_3d(N1, N2, N3, map_in_w, map_out1, FFTW_ESTIMATE))) {
    printf("FFT Problem...\n");
    exit(1);
  }
  fftw_execute(pr2c);
  ...
  fid_in=fopen(argv[8],"rb");	

  if(bsize==4) {
    if(!(map_in_f=(float *) malloc(N1*N2*N3*sizeof(float)))) {
      printf("Mem1b Problem...\n");
      exit(1);
    }
    if(fread(map_in_f,bsize,N1*N2*N3,fid_in)!= N1*N2*N3) {
      printf("Reading file error...\n");
...
#pragma omp parallel for shared(map_in_w,map_in_d,N1,N2,N3) private(i,j,p)
  ...
  if(!(map_out2 = (double complex*) malloc(sizeof(double complex) * N1*N2*(N3/2+1)))) {
    printf("Mem4b Problem...\n");
    exit(1);
  }
  if(!(pr2c=fftw_plan_dft_r2c_3d(N1, N2, N3, map_in_w, map_out2, FFTW_ESTIMATE))) {
    printf("FFTb Problem...\n");
    exit(1);
  }
  fftw_execute(pr2c);
  ...
  for(i=0;i<N1;i++) {
	...
	indk=(long int)round(k/dk);
        nps[indk]++;
        pwsp3d[indk]+=map_out1[i*N2*(N3/2+1)+j*(N3/2+1)+p]*conj(map_out2[i*N2*(N3/2+1)+j*(N3/2+1)+p]);
      }
    }
  }

printf("# Output: k(units 1/dl)     P(k)_real    P(k)_img (units of input box cell values^2*dl^3)\n");
  for(i=0;i<nk;i++) {
    if(nps[i]==0) nps[i]=1;
      printf("%f   %E    %E\n",i*dk,creal(pwsp3d[i])/nps[i]/wsum*dl*dl*dl,cimag(pwsp3d[i])/nps[i]/wsum*dl*dl*dl); 
  }
