...
int main(int argc, char **argv){
  ...
  if(!(map_in_w=(float *) malloc(N1*N2*N3*sizeof(float)))) {
...
#pragma omp parallel for shared(map_in_w,map_in_f,N1,N2,N3) private(i,j,p)
    ...
    for(i=0;i<N1;i++) {
      for(j=0;j<N2;j++) {
	for(p=0;p<N3;p++) {
	  map_in_w[i*N2*N3+j*N3+p]=(float)map_in_f[i*N2*N3+j*N3+p]*wind(i,j,p,N1-1,N2-1,N3-1,flag);  /* .....apply window to patch */
	}
      }
    }
    free(map_in_f);
...
#pragma omp parallel for shared(map_in_w,map_in_d,N1,N2,N3) private(i,j,p)
    ...
    for(i=0;i<N1;i++) {
      for(j=0;j<N2;j++) {
	for(p=0;p<N3;p++) {
	  map_in_w[i*N2*N3+j*N3+p]=(float)map_in_d[i*N2*N3+j*N3+p]*wind(i,j,p,N1-1,N2-1,N3-1,flag);  /* .....apply window to patch */
	}
      }
    }
    free(map_in_d);
  ...
  if(!(map_out = (float complex*) malloc(sizeof(float complex) * N1*N2*(N3/2+1)))) {
    printf("Mem3 Problem...\n");
    exit(1);
  }
  if(!(pr2c=fftwf_plan_dft_r2c_3d(N1, N2, N3, map_in_w, map_out, FFTW_ESTIMATE))) {
    printf("FFT Problem...\n");
    exit(1);
  }
  fftwf_execute(pr2c);
