...
#endif
  for(ind=0;ind<global_N3_halo;ind++) {
    map_in[ind]=map_in[ind]*growth_in +1.;
  }
...
#ifdef _OMPTHREAD_
#pragma omp parallel for shared(global_N3_halo,map_out) private(indice)
#endif
      for(indice=0;indice<global_N3_halo;indice++) {
	map_out[indice] = -1.0;
      }
      /***************************** CIC smoothing ******************************************************/
      //#ifdef _OMPTHREAD_
	...
	for(j=0;j<global_N_halo;j++){
      ...
      printf("Smoothing...\n");fflush(0);   
      for(i=0;i<(global_N3_smooth);i++) map_out2[i] =0.0;
	...
	sprintf(fname, "%s/delta/deltanl_z%.3f_N%ld_L%.0f.dat",argv[1],redshift,global_N_halo,global_L/global_hubble); 
	if((fid=fopen(fname,"wb"))==NULL){  
